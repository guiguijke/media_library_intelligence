...
class SonarrConnector:
    ...
    async def _request(self, method: str, endpoint: str, json_data: Optional[dict] = None, params: Optional[dict] = None) -> Optional[Any]:
        ...
        try:
            ...
            logger.error(f"Sonarr API HTTP error ({url}): {exc.response.status_code} - {exc.response.text}")
            raise ConnectorException(f"Sonarr API error: {exc.response.status_code}") from exc
        except httpx.RequestError as exc:
            logger.error(f"Sonarr API request error ({url}): {exc}")
            raise ConnectorException(f"Sonarr API request failed: {exc}") from exc
    ...
    async def add_series(self, title: str, tmdb_id: Optional[int] = None, tvdb_id: Optional[int] = None, quality_profile: Optional[int] = None, root_folder: Optional[str] = None) -> Optional[Dict[str, Any]]:
        # Search for the series first; prefer tvdb_id, fallback to tmdb_id
        search = None
        if tvdb_id:
            search = await self._request("GET", "/series/lookup", params={"term": f"tvdb:{tvdb_id}"})
        if not isinstance(search, list) or not search:
            term = f"tmdb:{tmdb_id}" if tmdb_id else title
            search = await self._request("GET", "/series/lookup", params={"term": term})
        if not isinstance(search, list) or not search:
        ...
        if not result or not result.get("id"):
            logger.warning(f"Sonarr: failed to add series {title}, response: {result}")
            return None
        return result
    ...
    async def get_queue_status(self) -> List[Dict[str, Any]]:
        ...
        while True:
            data = await self._request(
                "GET", "/queue", params={"page": page, "pageSize": page_size, "includeSeries": "true", "includeEpisode": "true"}
            )
