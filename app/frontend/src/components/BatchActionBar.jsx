...
import { useBatchActions, useRecommendations } from '../hooks/useMedia'
...
export default function BatchActionBar() {
  ...
  const { data } = useRecommendations({ category: 'all' })
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-2xl px-4">
      <div className="bg-surface-elevated border border-border rounded-xl shadow-2xl px-5 py-4 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="px-2.5 py-1 rounded-md bg-accent text-background text-sm font-bold">
            {count}
          </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-md bg-surface text-primary text-sm font-medium hover:bg-border transition-colors disabled:opacity-50"
          >
